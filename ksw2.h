...
void ksw_exts2_sse(void *km, int qlen, const uint8_t *query, int tlen,
                   ...
                   int zdrop, int8_t junc_bonus, int flag, const uint8_t *junc,
                   ksw_extz_t *ez);
...
static inline void ksw_backtrack(
    ...
    while (i >= 0 && j >= 0) { // at the beginning of the loop, _state_ tells us
        ...
        if (state == 0)
            state =
                tmp &
                7; // if requesting the H state, find state one maximizes it.
        else if (!(tmp >> (state + 2) & 1))
            state = 0; // if requesting other states, _state_ stays the same if
                       // it is a continuation; otherwise, set to H
        if (state == 0)
            state = tmp & 7; // TODO: probably this line can be merged into the
                             // "else if" line right above; not 100% sure
        ...
        if (state == 0)
            ...
            cigar =
                ksw_push_cigar(km, &n_cigar, &m_cigar, cigar, KSW_CIGAR_DEL, 1),
            --i;
            ...
            cigar =
                ksw_push_cigar(km, &n_cigar, &m_cigar, cigar, KSW_CIGAR_INS, 1),
            --j;
    }
    if (i >= 0)
        cigar = ksw_push_cigar(km, &n_cigar, &m_cigar, cigar,
                               min_intron_len > 0 && i >= min_intron_len
                                   ? KSW_CIGAR_N_SKIP
                                   : KSW_CIGAR_DEL,
                               i + 1); // first deletion
    if (j >= 0)
        cigar = ksw_push_cigar(km, &n_cigar, &m_cigar, cigar, KSW_CIGAR_INS,
                               j + 1); // first insertion
    if (!is_rev)
