...
static inline void update_max_zdrop(int32_t score, int i, int j, int32_t *max,
                                    int *max_i, int *max_j, int e,
...
static int mm_test_zdrop(void *km, const mm_mapopt_t *opt, const uint8_t *qseq,
                         const uint8_t *tseq, uint32_t n_cigar, uint32_t *cigar,
                         const int8_t *mat) {
    uint32_t k;
    int32_t  score = 0, max = INT32_MIN, max_i = -1, max_j = -1, i = 0, j = 0,
            max_zdrop = 0;
    ...
    for (k = 0, score = 0; k < n_cigar; ++k) {
        ...
        if (op == MM_CIGAR_MATCH) {
            for (l = 0; l < len; ++l) {
                ...
                update_max_zdrop(score, i + l, j + l, &max, &max_i, &max_j,
                                 opt->e, &max_zdrop, pos);
            }
            i += len, j += len;
            ...
            if (op == MM_CIGAR_INS)
                ...
                i += len;
            update_max_zdrop(score, i, j, &max, &max_i, &max_j, opt->e,
                             &max_zdrop, pos);
        }
    }
    ...
    if (!(opt->flag &
          (MM_F_SPLICE | MM_F_SR | MM_F_FOR_ONLY | MM_F_REV_ONLY)) &&
        ...
        score = ksw_ll_i16(qp, t_len, tseq + pos[ 0 ][ 0 ], opt->q, opt->e,
                           &q_off, &t_off);
        kfree(km, qseq2);
        kfree(km, qp);
...
static void mm_fix_cigar(mm_reg1_t *r, const uint8_t *qseq, const uint8_t *tseq,
    ...
    *qshift = *tshift = 0;
    ...
    for (k = 0; k < p->n_cigar; ++k) { // indel left alignment
        ...
        if (op == MM_CIGAR_MATCH) {
            ...
            if (k > 0 && k < p->n_cigar - 1 && (p->cigar[ k - 1 ] & 0xf) == 0 &&
                ...
                if (l > 0)
                    p->cigar[ k - 1 ] -= l << 4, p->cigar[ k + 1 ] += l << 4,
                        qoff -= l, toff -= l;
    ...
    for (k = 0; k < p->n_cigar - 2; ++k) { // fix CIGAR like 5I6D7I
        if ((p->cigar[ k ] & 0xf) > 0 &&
            (p->cigar[ k ] & 0xf) + (p->cigar[ k + 1 ] & 0xf) == 3) {
            uint32_t l, s[ 3 ] = {0, 0, 0};
            for (l = k; l < p->n_cigar;
                 ++l) { // count number of adjacent I and D
            ...
            if (s[ 1 ] > 0 && s[ 2 ] > 0 &&
                l - k > 2) { // turn to a single I and a single D
    ...
    if (to_shrink) { // squeeze out zero-length operations
        ...
        p->n_cigar = l;
        for (k = l = 0; k < p->n_cigar;
             ++k) // merge two adjacent operations if they are the same
            if (k == p->n_cigar - 1 ||
                (p->cigar[ k ] & 0xf) != (p->cigar[ k + 1 ] & 0xf))
                p->cigar[ l++ ] = p->cigar[ k ];
            else
                p->cigar[ k + 1 ] +=
                    p->cigar[ k ] >>
                    4 << 4; // add length to the next CIGAR operator
        p->n_cigar = l;
    }
    if ((p->cigar[ 0 ] & 0xf) == MM_CIGAR_INS ||
        (p->cigar[ 0 ] & 0xf) == MM_CIGAR_DEL) { // get rid of leading I or D
...
static void mm_update_cigar_eqx(mm_reg1_t *r, const uint8_t *qseq,
                                const uint8_t *tseq) // written by @armintoepfer
{
    uint32_t    n_EQX = 0;
    ...
    for (k = 0; k < r->p->n_cigar; ++k) {
        ...
        if (op == MM_CIGAR_MATCH) {
            while (len > 0) {
                for (l = 0; l < len && qseq[ qoff + l ] == tseq[ toff + l ];
                     ++l) {
    ...
    if (n_EQX == n_M) {
        for (k = 0; k < r->p->n_cigar; ++k) {
            uint32_t op = r->p->cigar[ k ] & 0xf, len = r->p->cigar[ k ] >> 4;
            if (op == MM_CIGAR_MATCH)
                r->p->cigar[ k ] = len << 4 | MM_CIGAR_EQ_MATCH;
        }
        return;
    }
...
static void mm_update_extra(mm_reg1_t *r, const uint8_t *qseq,
                            const uint8_t *tseq, const int8_t *mat, int8_t q,
                            int8_t e, int is_eqx, int log_gap) {
    uint32_t    k, l;
    ...
    qseq += qshift, tseq += tshift; // qseq and tseq may be shifted due to the
                                    // removal of leading I/D
    r->blen = r->mlen = 0;
    for (k = 0; k < p->n_cigar; ++k) {
        ...
        if (op == MM_CIGAR_MATCH) {
            ...
            r->blen += len - n_ambi, r->mlen += len - (n_ambi + n_diff),
                p->n_ambi += n_ambi;
            toff += len, qoff += len;
        } else if (op == MM_CIGAR_INS) {
            int n_ambi = 0;
    ...
    if (is_eqx)
        mm_update_cigar_eqx(r, qseq,
                            tseq); // NB: it has to be called here as changes to
                                   // qseq and tseq are not returned
}

static void
    mm_append_cigar(mm_reg1_t *r, uint32_t n_cigar,
                    uint32_t *cigar) // TODO: this calls the libc realloc()
{
    mm_extra_t *p;
    if (n_cigar == 0) return;
    if (r->p == 0) {
    ...
    } else if (r->p->n_cigar + n_cigar + sizeof(mm_extra_t) / 4 >
               r->p->capacity) {
    ...
    p = r->p;
    if (p->n_cigar > 0 &&
        (p->cigar[ p->n_cigar - 1 ] & 0xf) ==
            (cigar[ 0 ] & 0xf)) { // same CIGAR op at the boundary
        p->cigar[ p->n_cigar - 1 ] += cigar[ 0 ] >> 4 << 4;
        if (n_cigar > 1)
            memcpy(p->cigar + p->n_cigar, cigar + 1, (n_cigar - 1) * 4);
        p->n_cigar += n_cigar - 1;
    } else {
...
static void mm_align_pair(void *km, const mm_mapopt_t *opt, int qlen,
                          const uint8_t *qseq, int tlen, const uint8_t *tseq,
                          const uint8_t *junc, const int8_t *mat, int w,
                          int end_bonus, int zdrop, int flag, ksw_extz_t *ez) {
 
    if (mm_dbg_flag & MM_DBG_PRINT_ALN_SEQ) {
        int i;
        fprintf(stderr,
                "===> q=(%d,%d), e=(%d,%d), bw=%d, flag=%d, zdrop=%d <===\n",
    ...
    if (opt->max_sw_mat > 0 && (int64_t) tlen * qlen > opt->max_sw_mat) {
        ...
        ksw_exts2_sse(km, qlen, qseq, tlen, tseq, 5, mat, opt->q, opt->e,
                      opt->q2, opt->noncan, zdrop, opt->junc_bonus, flag, junc,
                      ez);
    }
    else if (opt->q == opt->q2 && opt->e == opt->e2) {
        ksw_extz2_sse(km, qlen, qseq, tlen, tseq, 5, mat, opt->q, opt->e, w,
                      zdrop, end_bonus, flag, ez);
    }
    else {
        ksw_extd2_sse(km, qlen, qseq, tlen, tseq, 5, mat, opt->q, opt->e,
                      opt->q2, opt->e2, w, zdrop, end_bonus, flag, ez);
    ...
    if (mm_dbg_flag & MM_DBG_PRINT_ALN_SEQ) {
        ...
        for (i = 0; i < ez->n_cigar; ++i)
            fprintf(stderr, "%d%c", ez->cigar[ i ] >> 4,
                    MM_CIGAR_STR[ ez->cigar[ i ] & 0xf ]);
        fprintf(stderr, "\n");
    }
...
static inline int mm_get_hplen_back(const mm_idx_t *mi, uint32_t rid,
                                    uint32_t x) {
...
static inline void mm_adjust_minier(const mm_idx_t *mi,
                                    uint8_t *const qseq0[ 2 ], mm128_t *a,
                                    int32_t *r, int32_t *q) {
    if (mi->flag & MM_I_HPC) {
...
static int *collect_long_gaps(void *km, int as1, int cnt1, mm128_t *a,
                              int min_gap, int *n_) {
    int i, n, *K;
    *n_ = 0;
    for (i = 1, n = 0; i < cnt1;
         ++i) { // count the number of gaps longer than min_gap
        int gap = ((int32_t) a[ as1 + i ].y - a[ as1 + i - 1 ].y) -
                  ((int32_t) a[ as1 + i ].x - a[ as1 + i - 1 ].x);
...
static void mm_filter_bad_seeds(void *km, int as1, int cnt1, mm128_t *a,
                                int min_gap, int diff_thres, int max_ext_len,
                                int max_ext_cnt) {
    ...
    for (k = 0;; ++k) { // traverse long gaps
        ...
        i   = K[ k ];
        gap = ((int32_t) a[ as1 + i ].y - (int32_t) a[ as1 + i - 1 ].y) -
              (int32_t)(a[ as1 + i ].x - a[ as1 + i - 1 ].x);
        if (gap > 0)
            n_ins += gap;
        else
        ...
        for (l = k + 1; l < n && l <= k + max_ext_cnt; ++l) {
            ...
            if ((int32_t) a[ as1 + j ].y - qs > max_ext_len ||
                (int32_t) a[ as1 + j ].x - rs > max_ext_len)
                break;
...
static void mm_filter_bad_seeds_alt(void *km, int as1, int cnt1, mm128_t *a,
                                    int min_gap, int max_ext) {
    int n, k, *K;
    K = collect_long_gaps(km, as1, cnt1, a, min_gap, &n);
    if (K == 0) return;
    for (k = 0; k < n;) {
...
static void mm_fix_bad_ends(const mm_reg1_t *r, const mm128_t *a, int bw,
                            int min_match, int32_t *as, int32_t *cnt) {
    int32_t i, l, m;
    *as = r->as, *cnt = r->cnt;
    if (r->cnt < 3) return;
...
static void mm_max_stretch(const mm_reg1_t *r, const mm128_t *a, int32_t *as,
                           int32_t *cnt) {
...
static int mm_seed_ext_score(void *km, const mm_mapopt_t *opt,
                             const mm_idx_t *mi, const int8_t mat[ 25 ],
                             int qlen, uint8_t *qseq0[ 2 ], const mm128_t *a) {
    uint8_t *qseq, *tseq;
    int q_span  = a->y >> 32 & 0xff, qs, qe, rs, re, rid, score, q_off, t_off,
    ...
    re = re + ext_len < (int32_t) mi->seq[ rid ].len ? re + ext_len
                                                     : mi->seq[ rid ].len;
...
static void mm_fix_bad_ends_splice(
    void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, const mm_reg1_t *r,
    const int8_t mat[ 25 ], int qlen, uint8_t *qseq0[ 2 ], const mm128_t *a,
    int *as1, int *cnt1) { // this assumes a very crude k-mer based mode; it is
                           // not necessary to use a good model just for
                           // filtering bounary exons
    int    score;
    double log_gap;
    *as1 = r->as, *cnt1 = r->cnt;
    if (r->cnt < 3) return;
    ...
    if ((a[ r->as ].y >> 32 & 0xff) < log_gap + opt->anchor_ext_shift) {
        ...
        if ((double) score / mat[ 0 ] <
            log_gap + opt->anchor_ext_shift) // a more exact format is "score <
                                             // log_4(gap) + shift"
            ++(*as1), --(*cnt1);
    }
    log_gap = log((int32_t) a[ r->as + r->cnt - 1 ].x -
                  (int32_t) a[ r->as + r->cnt - 2 ].x);
    if ((a[ r->as + r->cnt - 1 ].y >> 32 & 0xff) <
        log_gap + opt->anchor_ext_shift) {
        score = mm_seed_ext_score(km, opt, mi, mat, qlen, qseq0,
                                  &a[ r->as + r->cnt - 1 ]);
...
static void mm_align1(void *km, const mm_mapopt_t *opt, const mm_idx_t *mi,
                      int qlen, uint8_t *qseq0[ 2 ], mm_reg1_t *r,
                      mm_reg1_t *r2, int n_a, mm128_t *a, ksw_extz_t *ez,
                      int splice_flag) {
    int is_sr     = !!(opt->flag & MM_F_SR),
        is_splice = !!(opt->flag & MM_F_SPLICE);
    ...
    if (is_sr)
        assert(
            !(mi->flag & MM_I_HPC)); // HPC won't work with SR because with HPC
                                     // we can't easily tell if there is a gap
    ...
    if (is_sr && !(mi->flag & MM_I_HPC)) {
        ...
        if (!(opt->flag & MM_F_NO_END_FLT)) {
            if (is_splice)
                mm_fix_bad_ends_splice(km, opt, mi, r, mat, qlen, qseq0, a,
                                       &as1, &cnt1);
            else
                mm_fix_bad_ends(r, a, opt->bw, opt->min_chain_score * 2, &as1,
                                &cnt1);
        } else
            as1 = r->as, cnt1 = r->cnt;
    ...
    if (is_splice) {
        if (splice_flag & MM_F_SPLICE_FOR)
            extra_flag |= rev ? KSW_EZ_SPLICE_REV : KSW_EZ_SPLICE_FOR;
        if (splice_flag & MM_F_SPLICE_REV)
            extra_flag |= rev ? KSW_EZ_SPLICE_FOR : KSW_EZ_SPLICE_REV;
    ...
    if (is_sr) {
        ...
        l += l * opt->a + opt->end_bonus > opt->q
                 ...
                 : 0;
        re0 =
            re + l < (int32_t) mi->seq[ rid ].len ? re + l : mi->seq[ rid ].len;
    } else {
        // compute rs0 and qs0
        ...
        rs1 = qs1 = 0;
        for (i = r->as - 1, l = 0;
             i >= 0 && a[ i ].x >> 32 == a[ r->as ].x >> 32;
             --i) { // inspect nearby seeds
            int32_t x =
                (int32_t) a[ i ].x + 1 - (int32_t)(a[ i ].y >> 32 & 0xff);
            int32_t y =
                (int32_t) a[ i ].y + 1 - (int32_t)(a[ i ].y >> 32 & 0xff);
            if (x < rs0 && y < qs0) {
                if (++l > opt->min_cnt) {
                    l   = rs0 - x > qs0 - y ? rs0 - x : qs0 - y;
                    rs1 = rs0 - l, qs1 = qs0 - l;
                    if (rs1 < 0)
                        rs1 = 0; // not strictly necessary; better have this
                                 // guard for explicit
                    break;
                }
            }
        ...
        for (i = r->as + r->cnt, l = 0;
             i < n_a && a[ i ].x >> 32 == a[ r->as ].x >> 32;
             ++i) { // inspect nearby seeds
        ...
        if (qe < qlen && re < (int32_t) mi->seq[ rid ].len) {
            ...
            l += l * opt->a > opt->q ? (l * opt->a - opt->q) / opt->e : 0;
            l = l < opt->max_gap ? l : opt->max_gap;
            l = l < (int32_t) mi->seq[ rid ].len - re ? l
                                                      : mi->seq[ rid ].len - re;
    ...
    if (qs > 0 && rs > 0) { // left extension; probably the condition can be
                            // changed to "qs > qs0 && rs > rs0"
        if (opt->flag & MM_F_QSTRAND) {
            qseq = &qseq0[ 0 ][ qs0 ];
        ...
        mm_align_pair(
            km, opt, qs - qs0, qseq, rs - rs0, tseq, junc, mat, bw,
    ...
    for (i = is_sr ? cnt1 - 1 : 1; i < cnt1; ++i) { // gap filling
        if ((a[ as1 + i ].y & (MM_SEED_IGNORE | MM_SEED_TANDEM)) &&
            i != cnt1 - 1)
            continue;
        if (is_sr && !(mi->flag & MM_I_HPC)) {
        ...
        if (i == cnt1 - 1 || (a[ as1 + i ].y & MM_SEED_LONG_JOIN) ||
            ...
            if (is_sr) { // perform ungapped alignment
                ...
                ez->cigar = ksw_push_cigar(km, &ez->n_cigar, &ez->m_cigar,
                                           ez->cigar, MM_CIGAR_MATCH, qe - qs);
            } else { // perform normal gapped alignment
                mm_align_pair(km, opt, qe - qs, qseq, re - rs, tseq, junc, mat,
                              bw1, -1, opt->zdrop,
                              extra_flag | KSW_EZ_APPROX_MAX,
                              ez); // first pass: with approximate Z-drop
            }
            // test Z-drop and inversion Z-drop
            if ((zdrop_code = mm_test_zdrop(km, opt, qseq, tseq, ez->n_cigar,
                                            ez->cigar, mat)) != 0)
                mm_align_pair(km, opt, qe - qs, qseq, re - rs, tseq, junc, mat,
                              bw1, -1,
                              zdrop_code == 2 ? opt->zdrop_inv : opt->zdrop,
                              extra_flag, ez); // second pass: lift approximate
            // update CIGAR
            if (ez->n_cigar > 0) mm_append_cigar(r, ez->n_cigar, ez->cigar);
            if (ez->zdropped) { // truncated by Z-drop; TODO: sometimes Z-drop
                                // kicks in because the next seed placement is
                                // wrong. This can be fixed in principle.
                if (!r->p) {
                    assert(ez->n_cigar == 0);
    ...
    if (!dropped && qe < qe0 && re < re0) { // right extension
        ...
        mm_align_pair(km, opt, qe0 - qe, qseq, re0 - re, tseq, junc, mat, bw,
                      opt->end_bonus, opt->zdrop, extra_flag | KSW_EZ_EXTZ_ONLY,
                      ez);
        if (ez->n_cigar > 0) {
            mm_append_cigar(r, ez->n_cigar, ez->cigar);
            r->p->dp_score += ez->max;
...
static int mm_align1_inv(
    void *km, const mm_mapopt_t *opt, const mm_idx_t *mi, int qlen,
    uint8_t *qseq0[ 2 ], const mm_reg1_t *r1, const mm_reg1_t *r2,
    mm_reg1_t * r_inv,
    ...
    mm_align_pair(km, opt, ql - q_off, qseq + q_off, tl - t_off, tseq + t_off,
                  0, mat, (int) (opt->bw * 1.5), -1, opt->zdrop,
                  KSW_EZ_EXTZ_ONLY, ez);
...
static inline mm_reg1_t *mm_insert_reg(const mm_reg1_t *r, int i, int *n_regs,
                                       mm_reg1_t *regs) {
    regs = (mm_reg1_t *) realloc(regs, (*n_regs + 1) * sizeof(mm_reg1_t));
    if (i + 1 != *n_regs)
        memmove(&regs[ i + 2 ], &regs[ i + 1 ],
                sizeof(mm_reg1_t) * (*n_regs - i - 1));
    regs[ i + 1 ] = *r;
    ++*n_regs;
    return regs;
}

static inline void mm_count_gaps(const mm_reg1_t *r, int32_t *n_gap_,
                                 int32_t *n_gapo_) {
    uint32_t i;
...
static int32_t mm_recal_max_dp(const mm_reg1_t *r, double b2,
                               int32_t match_sc) {
    uint32_t i;
...
mm_reg1_t *mm_align_skeleton(void *km, const mm_mapopt_t *opt,
    ...
    for (i = 0; i < n_regs; ++i) {
        ...
        if ((opt->flag & MM_F_SPLICE) && (opt->flag & MM_F_SPLICE_FOR) &&
            (opt->flag & MM_F_SPLICE_REV)) { // then do two rounds of alignments
                                             // for both strands
            mm_reg1_t s[ 2 ], s2[ 2 ];
            int       which, trans_strand;
            s[ 0 ] = s[ 1 ] = regs[ i ];
            mm_align1(km, opt, mi, qlen, qseq0, &s[ 0 ], &s2[ 0 ], n_a, a, &ez,
                      MM_F_SPLICE_FOR);
            mm_align1(km, opt, mi, qlen, qseq0, &s[ 1 ], &s2[ 1 ], n_a, a, &ez,
                      MM_F_SPLICE_REV);
            if (s[ 0 ].p->dp_score > s[ 1 ].p->dp_score)
                ...
                trans_strand = 3,
                which        = (qlen + s[ 0 ].p->dp_score) &
                        1; // randomly choose a strand, effectively
            if (which == 0) {
                regs[ i ] = s[ 0 ], r2 = s2[ 0 ];
                free(s[ 1 ].p);
            } else {
            ...
            mm_align1(km, opt, mi, qlen, qseq0, &regs[ i ], &r2, n_a, a, &ez,
                      opt->flag);
        ...
        if (i > 0 && regs[ i ].split_inv && !(opt->flag & MM_F_NO_INV)) {
            if (mm_align1_inv(km, opt, mi, qlen, qseq0, &regs[ i - 1 ],
                              &regs[ i ], &r2, &ez)) {
    ...
    *n_regs_ = n_regs;
    ...
    if (!(opt->flag & MM_F_SR) && !opt->split_prefix &&
