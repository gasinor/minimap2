...
static mm128_t *compact_a(void *km, int32_t n_u, uint64_t *u, int32_t n_v,
                          int32_t *v, mm128_t *a) {
    mm128_t * b, *w;
    uint64_t *u2;
    int64_t   i, j, k;
...
mm128_t *mg_lchain_dp(
    ...
    for (i = 0, max_ii = -1; i < n; ++i) {
        ...
        while (st < i && (a[ i ].x >> 32 != a[ st ].x >> 32 ||
                          a[ i ].x > a[ st ].x + max_dist_x))
            ++st;
        if (i - st > max_iter) st = i - max_iter;
        for (j = i - 1; j >= st; --j) {
            int32_t sc;
            sc = comput_sc(&a[ i ], &a[ j ], max_dist_x, max_dist_y, bw,
        ...
        v[ i ] = max_j >= 0 && v[ max_j ] > max_f
                     ? v[ max_j ]
                     : max_f; // v[] keeps the peak score up to i; f[] is the
                              // score ending at i, not always the peak
        if (max_ii < 0 || (a[ i ].x - a[ max_ii ].x <= (int64_t) max_dist_x &&
                           f[ max_ii ] < f[ i ]))
            max_ii = i;
...
mm128_t *mg_lchain_rmq(int max_dist, int max_dist_inner, int bw,
    ...
    for (i = i0 = 0; i < n; ++i) {
            ...
            if (!exact && root_inner && (int32_t) a[ i ].y > 0) {
                ...
                if (lo) {
                    ...
                    while ((q = krmq_at(&itr)) != 0) {
                        ...
                        ++n_rmq_iter;
                        j  = q->i;
                        sc = f[ j ] + comput_sc_simple(&a[ i ], &a[ j ],
                                                       chn_pen_gap,
        ...
        // set max
        assert(max_j < 0 || (a[ max_j ].x < a[ i ].x &&
                             (int32_t) a[ max_j ].y < (int32_t) a[ i ].y));
        f[ i ] = max_f, p[ i ] = max_j;
        v[ i ] = max_j >= 0 && v[ max_j ] > max_f
                     ? v[ max_j ]
                     : max_f; // v[] keeps the peak score up to i; f[] is the
