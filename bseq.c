...
// use kseq reading files and use bseq store
KSEQ_INIT2(, gzFile, gzread)  // here generate all functions and variables
...
static inline void kseq2bseq(kseq_t *ks, mm_bseq1_t *s, int with_qual,
    ...
    for (i = 0; i < (int) ks->seq.l; ++i) // convert U to T
        if (s->seq[ i ] == 'u' || s->seq[ i ] == 'U') --s->seq[ i ];
...
mm_bseq1_t *mm_bseq_read3(mm_bseq_file_t *fp, int64_t chunk_size, int with_qual,
    ...
    *n_                  = 0;
    ...
    if (ret < -1) {
        if (a.n)
            fprintf(stderr,
                    "[WARNING]\033[1;31m failed to parse the FASTA/FASTQ "
                    "record next to '%s'. Continue anyway.\033[0m\n",
                    a.a[ a.n - 1 ].name);
        else
            fprintf(stderr, "[WARNING]\033[1;31m failed to parse the first "
                            "FASTA/FASTQ record. Continue anyway.\033[0m\n");
    }
    *n_ = a.n;
    return a.a;
}
...
mm_bseq1_t *mm_bseq_read_frag2(int n_fp, mm_bseq_file_t **fp,
    ...
    *n_                  = 0;
    ...
    while (1) {
        int n_read = 0;
        for (i = 0; i < n_fp; ++i)
            if (kseq_read(fp[ i ]->ks) >= 0) ++n_read;
        if (n_read < n_fp) {
            if (n_read > 0)
                fprintf(stderr,
                        "[W::%s]\033[1;31m query files have different number "
                        "of records; extra records skipped.\033[0m\n",
                        __func__);
