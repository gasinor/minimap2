...
static inline void yes_or_no(mm_mapopt_t *opt, int64_t flag, int long_idx,
    ...
    if (yes_to_set) {
        if (strcmp(arg, "yes") == 0 || strcmp(arg, "y") == 0)
            ...
            fprintf(stderr,
                    "[WARNING]\033[1;31m option '--%s' only accepts 'yes' or "
                    "'no'.\033[0m\n",
...
int main(int argc, char *argv[]) {
    const char *opt_str = "2aSDw:k:K:t:r:f:Vv:g:G:I:d:XT:s:x:Hcp:M:n:z:A:B:O:E:"
                          "m:N:Qu:R:hF:LC:yYPo:e:U:j:";
    ...
    while ((c = ketopt(&o, argc, argv, 1, opt_str, long_options)) >= 0) {
        if (c == 'w')
            ipt.w = atoi(o.arg), ipt.flag &= ~MM_I_SYNCMER;
        else if (c == 'j')
            ipt.w = atoi(o.arg), ipt.flag |= MM_I_SYNCMER;
        else if (c == 'k')
            ipt.k = atoi(o.arg);
            ...
            if (strcmp(o.arg, "-") != 0) {
                if (freopen(o.arg, "wb", stdout) == NULL) {
                    fprintf(stderr,
                            "[ERROR]\033[1;31m failed to write the output to "
                            "file '%s'\033[0m: %s\n",
            ...
            mm_dbg_flag |= MM_DBG_PRINT_QNAME | MM_DBG_PRINT_SEED,
                n_threads = 1; // --print-seed
        else if (c == 307)
            ...
            fprintf(stderr, "[WARNING] \033[1;31m --lj-min-ratio has been "
                            "deprecated.\033[0m\n");
        } else if (c == 314) { // --frag
            ...
            if (o.arg == 0 || strcmp(o.arg, "short") == 0) {
                ...
                fprintf(
                    stderr,
                    "[WARNING]\033[1;31m --cs only takes 'short' or 'long'. "
                    "Invalid values are assumed to be 'short'.\033[0m\n");
            }
            ...
            if (mm_verbose >= 2)
                fprintf(stderr, "[WARNING]\033[1;31m option -S is deprecated "
                                "and may be removed in future. Please use "
                                "--cs=long instead.\033[0m\n");
        } else if (c == 'V') {
            puts(MM_VERSION);
            return 0;
        } else if (c == 'r') {
            ...
            if (*o.arg == 'b')
                ...
                opt.flag |= MM_F_SPLICE_FOR,
                    opt.flag &= ~MM_F_SPLICE_REV; // match GT-AG
            else if (*o.arg == 'r')
                opt.flag |= MM_F_SPLICE_REV,
                    opt.flag &= ~MM_F_SPLICE_FOR; // match CT-AC (reverse
                                                  // complement of GT-AG)
            else if (*o.arg == 'n')
                opt.flag &=
                    ~(MM_F_SPLICE_FOR |
                      MM_F_SPLICE_REV); // don't try to match the GT-AG signal
            else {
                fprintf(
                    stderr,
                    "[ERROR]\033[1;31m unrecognized cDNA direction\033[0m\n");
                return 1;
            }
        } else if (c == 'z') {
    ...
    if ((opt.flag & MM_F_SPLICE) && (opt.flag & MM_F_FRAG_MODE)) {
        fprintf(stderr, "[ERROR]\033[1;31m --splice and --frag should not be "
                        "specified at the same time.\033[0m\n");
        return 1;
    }
    ...
    if (opt.best_n == 0) {
        fprintf(stderr,
                "[WARNING]\033[1;31m changed '-N 0' to '-N %d "
                "--secondary=no'.\033[0m\n",
                old_best_n);
    ...
    if (argc == o.ind || fp_help == stdout) {
        fprintf(fp_help, "Usage: minimap2 [options] <target.fa>|<target.idx> "
                         "[query.fa] [...]\n");
        fprintf(fp_help, "Options:\n");
        fprintf(fp_help, "  Indexing:\n");
        fprintf(fp_help, "    -H           use homopolymer-compressed k-mer "
                         "(preferrable for PacBio)\n");
        fprintf(fp_help,
                "    -k INT       k-mer size (no larger than 28) [%d]\n",
                ipt.k);
        fprintf(fp_help, "    -w INT       minimizer window size [%d]\n",
                ipt.w);
        fprintf(fp_help,
                "    -j INT       syncmer submer size (overriding -w) []\n");
        fprintf(
            fp_help,
        ...
        fprintf(fp_help,
                "    -f FLOAT     filter out top FLOAT fraction of repetitive "
                "minimizers [%g]\n",
                opt.mid_occ_frac);
        fprintf(fp_help,
                "    -g NUM       stop chain enlongation if there are no "
                "minimizers in INT-bp [%d]\n",
                opt.max_gap);
        fprintf(fp_help, "    -G NUM       max intron length (effective with "
                         "-xsplice; changing -r) [200k]\n");
        fprintf(fp_help, "    -F NUM       max fragment length (effective with "
                         "-xsr or in the fragment mode) [800]\n");
        fprintf(fp_help,
                "    -r NUM[,NUM] chaining/alignment bandwidth and long-join "
                "bandwidth [%d,%d]\n",
                opt.bw, opt.bw_long);
        fprintf(
            fp_help,
            "    -n INT       minimal number of minimizers on a chain [%d]\n",
            opt.min_cnt);
        fprintf(fp_help,
                "    -m INT       minimal chaining score (matching bases minus "
                "log gap penalty) [%d]\n",
                opt.min_chain_score);
        //		fprintf(fp_help, "    -T INT       SDUST threshold; 0 to disable
        //SDUST [%d]\n", opt.sdust_thres); // TODO: this option is never used;
        //might be buggy
        fprintf(fp_help, "    -X           skip self and dual mappings (for "
                         "the all-vs-all mode)\n");
        fprintf(fp_help,
                "    -p FLOAT     min secondary-to-primary score ratio [%g]\n",
                opt.pri_ratio);
        fprintf(
            fp_help,
            "    -N INT       retain at most INT secondary alignments [%d]\n",
            opt.best_n);
        fprintf(fp_help, "  Alignment:\n");
        fprintf(fp_help, "    -A INT       matching score [%d]\n", opt.a);
        fprintf(fp_help,
                "    -B INT       mismatch penalty (larger value for lower "
                "divergence) [%d]\n",
                opt.b);
        ...
        fprintf(fp_help,
                ...
                "min{O1+k*E1,O2+k*E2} [%d,%d]\n",
                opt.e, opt.e2);
        fprintf(fp_help,
                "    -z INT[,INT] Z-drop score and inversion Z-drop score "
                "[%d,%d]\n",
                opt.zdrop, opt.zdrop_inv);
        fprintf(fp_help,
                "    -s INT       minimal peak DP alignment score [%d]\n",
                opt.min_dp_max);
        fprintf(fp_help, "    -u CHAR      how to find GT-AG. f:transcript "
                         "strand, b:both strands, n:don't match GT-AG [n]\n");
        ...
        fprintf(fp_help, "    -R STR       SAM read group line in a format "
                         "like '@RG\\tID:foo\\tSM:bar' []\n");
        fprintf(fp_help, "    -c           output CIGAR in PAF\n");
        fprintf(fp_help, "    --cs[=STR]   output the cs tag; STR is 'short' "
                         "(if absent) or 'long' [none]\n");
        fprintf(fp_help, "    --MD         output the MD tag\n");
        fprintf(fp_help, "    --eqx        write =/X CIGAR operators\n");
        fprintf(fp_help, "    -Y           use soft clipping for supplementary "
                         "alignments\n");
        ...
        fprintf(fp_help, "    -x STR       preset (always applied before other "
                         "options; see minimap2.1 for details) []\n");
        fprintf(fp_help, "                 - map-pb/map-ont - PacBio "
                         "CLR/Nanopore vs reference mapping\n");
        fprintf(fp_help, "                 - map-hifi - PacBio HiFi reads vs "
                         "reference mapping\n");
        fprintf(fp_help, "                 - ava-pb/ava-ont - PacBio/Nanopore "
                         "read overlap\n");
        fprintf(fp_help, "                 - asm5/asm10/asm20 - asm-to-ref "
                         "mapping, for ~0.1/1/5%% sequence divergence\n");
        fprintf(fp_help, "                 - splice/splice:hq - "
                         "long-read/Pacbio-CCS spliced alignment\n");
        fprintf(fp_help,
                "                 - sr - genomic short-read mapping\n");
        fprintf(fp_help, "\nSee `man ./minimap2.1' for detailed description of "
                         "these and other advanced command-line options.\n");
        return fp_help == stdout ? 0 : 1;
    }

    if ((opt.flag & MM_F_SR) && argc - o.ind > 3) {
        fprintf(stderr, "[ERROR] incorrect input: in the sr mode, please "
                        "specify no more than two query files.\n");
        return 1;
    }
    ...
    if (!idx_rdr->is_idx && fnw == 0 && argc - o.ind < 2) {
        fprintf(stderr, "[ERROR] missing input: please specify a query file to "
                        "map or option -d to keep the index\n");
    ...
    if (opt.best_n == 0 && (opt.flag & MM_F_CIGAR) && mm_verbose >= 2)
        fprintf(
            stderr,
            "[WARNING]\033[1;31m `-N 0' reduces alignment accuracy. Please use "
            "--secondary=no to suppress secondary alignments.\033[0m\n");
    while ((mi = mm_idx_reader_read(idx_rdr, n_threads)) != 0) {
        int ret;
        if ((opt.flag & MM_F_CIGAR) && (mi->flag & MM_I_NO_SEQ)) {
            fprintf(stderr,
                    "[ERROR] the prebuilt index doesn't contain sequences.\n");
            mm_idx_destroy(mi);
            mm_idx_reader_close(idx_rdr);
            return 1;
        }
        if ((opt.flag & MM_F_OUT_SAM) && idx_rdr->n_parts == 1) {
            if (mm_idx_reader_eof(idx_rdr)) {
                ...
                if (opt.split_prefix == 0 && mm_verbose >= 2)
                    fprintf(stderr, "[WARNING]\033[1;31m For a multi-part "
                                    "index, no @SQ lines will be outputted. "
                                    "Please use --split-prefix.\033[0m\n");
            }
            if (ret != 0) {
                mm_idx_destroy(mi);
                mm_idx_reader_close(idx_rdr);
        ...
        if (mm_verbose >= 3)
            fprintf(stderr,
                    "[M::%s::%.3f*%.2f] loaded/built the index for %d target "
                    "sequence(s)\n",
                    __func__, realtime() - mm_realtime0,
                    cputime() / (realtime() - mm_realtime0), mi->n_seq);
    ...
    if (opt.split_prefix)
        mm_split_merge(argc - (o.ind + 1), (const char **) &argv[ o.ind + 1 ],
                       &opt, n_parts);
